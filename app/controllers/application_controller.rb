...
class ApplicationController < ActionController::Base
  ...
  def shared_with_other_user(list_id, user)
    @swapitem = SwapItem.where("(receiver_listing_id = ? or offerer_listing_id = ?) and (offerer_id = ? or receiver_id = ?) and acceptance = true", list_id, list_id, user, user).first
    if @swapitem.nil?
      return false
    else
      return true
