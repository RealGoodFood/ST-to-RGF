class Listing < ActiveRecord::Base
  ...
  has_many :diets, :through => :diet_listings
  ...
  accepts_nested_attributes_for :origin_loc, :destination_loc#, :diets
...
#  attr_accessible :diet_list
  ...
  #tagged_with
  def self.tagged_with_diet_lists(name)
    Diet.find_by_name!(name).listings
  end

  #tag_counts
  def self.listing_diet_tag_counts
    Diet.select("diets.*, count(diet_listings.diet_id) as count").
      joins(:diet_listings).group("diet_listings.diet_id")
  end
  
  #tag_list
  def diet_tag_names
    @diet_tag_names || diets.map(&:name).join(", ")
  end
...
#    end
    ...
    unless current_user.restrictions.empty?
      current_user.restrictions.each do |r|
        unless current_user.restrictions.last.id == r.id
          str = "#{r.diet_id},"
        else
          str = "#{r.diet_id}"
        end
...
#    end 
  ...
  def available_now?
   (!valid_from.nil? || !valid_from.blank?) && (!valid_from.nil? || !valid_from.blank?) && (valid_from < DateTime.now && valid_until > DateTime.now)
  end
  ...
  def assign_diet_tags
    if @diet_tag_names
      self.diets = @diet_tag_names.split(",").map do |n|
        Diet.find_or_create_by_name(:name => n.strip )
      end
    end
  end
