module ApplicationHelper
  ...
  def self.store_community_service_name_to_thread_from_host(host=nil)
      ser_name = APP_CONFIG.global_service_name || "RGF"
  ...
  def self.fetch_community_service_name_from_thread
    Thread.current[:current_community_service_name] || APP_CONFIG.global_service_name || "RGF"
  end
  ...
  def popover_displayed
    if ((Time.now - @current_user.current_sign_in_at).abs/60).round > 30
      return false
    else
      return true
    end
  ...
  def just_login
    if ((Time.now - @current_user.current_sign_in_at).abs).round < 5
      return true
    end
  end
